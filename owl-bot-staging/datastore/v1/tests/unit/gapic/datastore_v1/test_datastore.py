...
from google.cloud.datastore_v1.services.datastore.transports.base import _GOOGLE_AUTH_VERSION
...
def test_datastore_client_service_account_always_use_jwt(client_class):
...
def test_datastore_client_service_account_always_use_jwt_true(transport_class, transport_name):
    with mock.patch.object(service_account.Credentials, 'with_always_use_jwt_access', create=True) as use_jwt:
        creds = service_account.Credentials(None, None, None)
...
def test_datastore_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_lookup_flattened():
    ...
    with mock.patch.object(
        ...
        assert args[0].read_options == datastore.ReadOptions(read_consistency=datastore.ReadOptions.ReadConsistency.STRONG)
        assert args[0].keys == [entity.Key(partition_id=entity.PartitionId(project_id='project_id_value'))]
...
def test_begin_transaction(transport: str = 'grpc', request_type=datastore.BeginTransactionRequest):
    client = DatastoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_commit_flattened():
    ...
    with mock.patch.object(
        ...
        assert args[0].mutations == [datastore.Mutation(insert=entity.Entity(key=entity.Key(partition_id=entity.PartitionId(project_id='project_id_value'))))]
...
def test_allocate_ids(transport: str = 'grpc', request_type=datastore.AllocateIdsRequest):
    client = DatastoreClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_datastore_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.datastore_v1.services.datastore.transports.DatastoreTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (ga_credentials.AnonymousCredentials(), None)
...
@requires_google_auth_lt_1_25_0
def test_datastore_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, 'default', autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        DatastoreClient()
...
@requires_google_auth_lt_1_25_0
def test_datastore_transport_auth_adc_old_google_auth(transport_class):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
